...
def setup_logger(args, mode=None):
    # create logger
    logger = logging.getLogger(__package__)
    ...
    if mode == "train":
        ...
        log_dir = os.path.join(args.basedir, args.expname, 'render_test_{}'.format(os.path.basename(args.ckpt_path)[:-4]), "{:03d}".format(args.style_ID) if args.stage == "second" else "")
        os.makedirs(log_dir, exist_ok=True)
    fileHandler = logging.FileHandler(os.path.join(log_dir, mode+"_log.txt"), 'w', 'utf-8')
...
def render_single_image(rank, world_size, models, ray_sampler, chunk_size, test_seed=None, style_ID=None):
    ...
    style_img, style_idx = ray_sampler.get_style_input(mode="test", test_seed=test_seed, style_ID=style_ID)
    for s in range(len(ray_batch_split['ray_d'])):
        ...
        for m in range(models['cascade_level']):
            ...
            with torch.no_grad():
                ret = net(ray_o, ray_d, fg_far_depth, fg_depth, bg_depth, style_img, style_idx)
...
def create_nerf(rank, args):
    ...
    for m in range(models['cascade_level']):
        ...
        net = StyleNeRFpp(args).to(rank)
        logger.info('Net work architecture: \n {}'.format(net))
    ...
    # Load pretrain weight for second stage
    if args.stage == "second":
        param_fixed = []
        logger.info("Loading pretrained weights from {}".format(args.first_stage_weight))
        map_location = {'cuda:%d' % 0: 'cuda:%d' % rank}
        to_load = torch.load(args.first_stage_weight, map_location=map_location)
        for m in range(models['cascade_level']):
            model_dict = models['net_{}'.format(m)].state_dict()
            state_dict = {k:v for k,v in to_load['net_{}'.format(m)].items() if k in model_dict.keys()}
            model_dict.update(state_dict)
            models['net_{}'.format(m)].load_state_dict(model_dict)
             
            for name, param in models['net_{}'.format(m)].named_parameters():
...
def ddp_train_nerf(rank, args):
    ...
    if rank == 0:
        ...
        backup_dir = os.path.join(args.basedir, args.expname, "backup")
        os.system("rm -rf {}".format(backup_dir))
        os.makedirs(backup_dir, exist_ok=True)
        copyfile("./ddp_train_nerf.py", backup_dir + "/ddp_train_nerf.py")
        copyfile("./utils.py", backup_dir + "/utils.py")
        copytree("./model", backup_dir + "/model/")
        copytree("./dataset", backup_dir + "/dataset/")
    ...
    for global_step in range(start+1, start+1+args.N_iters):
        ...
        ray_batch = ray_samplers[i].random_sample(args.N_rand, args.stage, center_crop=False)
        for key in ray_batch:
            if torch.is_tensor(ray_batch[key]):
        ...
        style_img, style_idx = ray_samplers[i].get_style_input()
        for m in range(models['cascade_level']):
            ...
            optim.zero_grad()
            ret = net(ray_batch['ray_o'], ray_batch['ray_d'], fg_far_depth, fg_depth, bg_depth, style_img, style_idx)
            all_rets.append(ret)
            ...
            if args.stage == "first":
                ...
                output_style_feats, output_style_feat_mean_std = net.module.get_style_feat(ret['rgb'].transpose(1, 0).reshape(3, 67, 81).unsqueeze(0))
                ...
                scalars_to_log['level_{}/C_loss'.format(m)] = content_loss.item()
                scalars_to_log['level_{}/S_loss'.format(m)] = style_loss.item()
        ...
        if global_step % args.i_img == 0 or global_step == start+1:
            ...
            log_data = render_single_image(rank, args.world_size, models, val_ray_samplers[idx], args.chunk_size, test_seed=global_step)
            what_val_to_log += 1
            dt = time.time() - time0
            if rank == 0:    # only main process should do this
                logger.info('Logged a random validation view in {} seconds'.format(dt))
                log_view_to_tb(writer, global_step, log_data, gt_img=val_ray_samplers[idx].get_img(), style_img=val_ray_samplers[idx].get_style_img(), mask=None, prefix='val/')

            time0 = time.time()
            idx = what_train_to_log % len(ray_samplers)
            log_data = render_single_image(rank, args.world_size, models, ray_samplers[idx], args.chunk_size, test_seed=global_step)
            what_train_to_log += 1
            dt = time.time() - time0
            if rank == 0:   # only main process should do this
                logger.info('Logged a random training view in {} seconds'.format(dt))
                log_view_to_tb(writer, global_step, log_data, gt_img=ray_samplers[idx].get_img(), style_img=ray_samplers[idx].get_style_img(), mask=None, prefix='train/')
...
def config_parser():
    ...
    parser.add_argument("--chunk_size", type=int, default=1024 * 2,  # 1024* 8
    ...
    # For hyper-network input
    parser.add_argument("--stage", type=str, default="first", help='which stage')
    parser.add_argument("--style_ID", type=int, default=None, help='style_ID for testing')
    parser.add_argument("--first_stage_weight", type=str, default=None, help='pretrained weight from first stage')
    parser.add_argument("--vgg_pretrained_path", type=str, default="./pretrained_StyleVAE/vgg_normalised.pth", help='VGG pretrained weights')
    parser.add_argument("--fc_encoder_pretrained_path", type=str, default="./pretrained_StyleVAE/fc_encoder_iter_160000.pth", help='FC encoder pretrained weights')
